          ...
          do ip=1,nproc_pool
            ...
            do iw = 1, nfreq
              ...
              call zgemm('C','N',nP,nPi,n, &
                         (1.d0,0.d0),Lia,size(Lia,1),Ria,size(Ria,1), &
...
#if defined(__CUDA)
  SUBROUTINE rpa_cholesky_gpu(edrpa,dfft,hamil_esh5,orbmat_esh5,esosex) 
    !
    USE constants, ONLY: tpi
    USE parallel_include
    USE wvfct, ONLY: wg, et
...
! check Ehf and Emp2 by direct calculation in 2x1x1 case
          ...
          do ip=1,nproc_pool
            ...
            ! broadcast Lia 
            if( me_pool+1 == ip ) Ria(:,:) = Lia(:,:) 
            if(nproc_pool>1) call mp_bcast( Ria, ip-1, intra_pool_comm )
...
! pack T1 differently to allow a single gemm call
    ...
    !if(allocated()) deallocate()
    !
  END SUBROUTINE rpa_cholesky_gpu
#endif
  !  
...
!    if(get_sosex) esosex=(0.d0,0.d0)
  ...
  SUBROUTINE rpa_process_Q(naux,iw,Qpq,Tpq,edrpa,edmp2) 
    ...
    dx=1.d0
    if(nspin == 1) dx = 2.d0 
    ! scale by spin factor, weight of frequency and factor of 2*pi
    dx = dx * iw / tpi
    ...
    do i=1,naux
      ...
      do j=1,naux
        edmp2 = edmp2 - dx * Qpq(i,j) * Qpq(j,i)  
      enddo 
    enddo 
    !
    allocate(ipiv(naux))
